...
<section>
    <div class="container">
        ...
        <div class="alert alert-success text-center" role="alert">
            Usuário interno criado com sucesso
        </div>
        <?php } ?>
        <div class="row">
            <div class="col-lg-12">
                <h1>Criar um usuário interno</h1>
            </div>
        </div>
        ...
        <div class="row linha_form">
            <form action="../Controller/CadastrarUser_interno.php" method="POST">
                <div class="col-lg-12">
                    ...
                    <select class="input_user" name="tipo_area">
                        <option value="0">Escolher</option>
                        <?php 
                            $sql_area = "SELECT * FROM tb_tipo_area";
                            $query_area = $mysqli->query($sql_area);

                            while($option = $query_area->fetch_assoc()){
                                echo '<option value="'.$option['tipo'].'">'.$option['nome_area'].'</option>';
                            }
                        ?>
                    </select>
